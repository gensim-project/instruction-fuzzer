...
static const uint32_t printf_buffer_size = 0x1000;
static char printf_buffer[0x1000];
...
int printf(const char *fmt, ...)
{
	...
	va_start(args, fmt);
	rc = vsnprintf(printf_buffer, printf_buffer_size, fmt, args);
	va_end(args);
	
	angel_write0(printf_buffer);
...
static int append_num(char *buffer, int size, uint64_t value, int base, int sgn, int pad, char pad_char)
{
	int n = 0;
