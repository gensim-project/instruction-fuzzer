...
static void GenerateTestFunction(size_t size)
{
	...
	std::cout << "Test function for size " << size << " at 0x" << std::hex << (uint64_t)(void*)tf->function << " (0x" << std::hex << tf->test_slot << ")" << std::endl;
}
...
std::string HarnessFormatResult(const Descriptor &test, const Descriptor &result)
{
	...
	for(int i = 0; i < 64; ++i) {
		if(memcmp(&data.input.sprs[i], &data.output.sprs[i], 4)) {
			str << "F" << std::dec << i << " " << std::hex << std::setw(8) << std::setfill('0') << bc_float(data.input.sprs[i]) << " -> " <<  std::setw(8) << std::setfill('0') << bc_float(data.output.sprs[i]) << std::endl;
		}
	}
